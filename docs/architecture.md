...
| `status` | `'pending' \| 'approved' \| 'rejected' \| 'used' \| 'demo'` | moderation lifecycle; `used` questions are never redrawn. `demo` sits outside it: the onboarding sample (`DEMO_QUESTION_ID`), never moderated, never drawn — the draw reads the `approved` pot — and the one status an anonymous visitor may read |
...
It is **laid over the app, not routed into it**. `src/App.tsx` renders it beside the navigator the way it renders the username sheet, for the same reason: there is no session yet, so there is nothing to navigate under. Whether it has been through is kept on the **device** (`AsyncStorage`, `useOnboardingSeen`) and not on the account — it runs before there is one — and storage that cannot be read counts as « already seen », since a carousel is not worth risking on every launch. The splash screen is held until that flag resolves as well as the session.

Three things it does that are worth knowing:

- **The demo question is real.** `v1_questions/{DEMO_QUESTION_ID}`, read once by its fixed id — the app's only Firestore read that runs with no session, which is what the `demo` rule above exists for. It is posed in the same red sheet as the daily question, with the same double tap and the same result under it, and it **writes nothing**: there is no account, no day and no streak to move, and the rules would refuse the answer anyway. The shares come from the tally the question already carries, which is why `npm run seed-demo-question` puts one there. A question that cannot be read is a carousel that ends on its sign-up button — never a step somebody is stuck on.
- **The permission dialog is primed, not sprung.** The fourth slide says why the 07:00 notification matters and its button is what raises the system dialog (`requestPushPermission`, beside the registration in `src/notifications/`). A refusal is final on both platforms, so the cold prompt at the first signed-in launch was the one thing worth moving. Nothing is registered there — no UID yet — and sign-in's own `registerDeviceForPush` then finds the permission granted and goes straight to the token.
