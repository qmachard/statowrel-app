...
- The Stats screen (docs/prd.md §5.2) renders from fixtures, not Firestore — `apps/app/src/stats/data/` holds three fake data sets, today's daily question included, and the `__DEV__` switch between them. Its daily-question banner and its calendar cells both open the day's question, so a fixture day and the real `v1_daily_questions` document behind it can disagree until that hook reads Firestore.
