...
`src/daily-question/` is the first screen reading Firestore for real (Stats is still on fixtures). The route is `DailyQuestion`, presented over Stats with an optional `date` param — no param means today, and today is the Europe/Paris day key, never the device's. `useDailyQuestion` reads three documents, because the model deliberately does not denormalize the question into the day: `v1_daily_questions/{date}`, the `v1_questions` document it points at, and the current user's answer — whose id *is* their UID, so "already answered" is one `get()` rather than a query. It reads the question only once the day has dropped, matching the rule above, and the four dead ends (`loading` / `unpublished` / `missing` / `error`) are rendered as states of the screen rather than swallowed.
...
- The question screen shows the options; it cannot take one. The double tap of docs/prd.md §4.3 (`expo-haptics` is not a dependency yet), the answer write and the StatOwrel card it flips to (§5.5) are all still to come — and until they are, the screen is a dismissable modal rather than the sheet §5.4 wants pinned open on today's unanswered question: trapping the user on a screen with no way to answer would be worse.
