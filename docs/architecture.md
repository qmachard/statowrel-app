...
The friend list of docs/prd.md §5.3 now reads those halves, on the Menu screen (`src/friends/`) — the invitation included, since both halves exist from the moment it is sent. Answering one is still to build: `firestore.rules` allows the accept (`pending` → `accepted`, never by the sender) and the delete that covers refusing, cancelling and removing alike, and nothing calls either yet.
