...
Components apply the shadows through `apps/app/src/design/shadows.ts`, which hands React Native the CSS `boxShadow` string (RN 0.76+) rather than the *legacy* iOS shadow props (`shadowOffset` / `shadowRadius` / `shadowOpacity`, plus `elevation` on Android). Those stop reproducing a CSS box-shadow faithfully once a surface has a corner radius — the edge softens, which defeats the point of a neobrutalist shadow — whereas `boxShadow` keeps a `0` blur radius at `0` on any radius. Fonts load via `expo-font` + `@expo-google-fonts/archivo-black` + `@expo-google-fonts/space-grotesk` in `apps/app/src/App.tsx`, with the native splash screen held until `useFonts` resolves. It then hands over to `apps/app/src/splash/Splash.tsx` — the star of `assets/lottie/star.json` over the brand background — which covers the app until the star has played and the session is restored. The two are configured to be one screen: `app.config.ts` gives the native splash the same background and the same star, still, at the same size and in the same frame, and the composition opens on the star already in place. The app icon is that same mark (`assets/icon.svg` → `icon.png`, plus the Android foreground layer), traced from the same contours as `StarPeeled`.
