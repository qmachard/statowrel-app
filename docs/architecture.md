...
| `answers_count` | `number` | total days answered since sign-up, catch-ups included — the Stats tile reads it rather than counting, since the calendar only ever loads one month |
...
`src/daily-question/` is the first screen reading Firestore for real (Stats is still on fixtures). The route is `DailyQuestion`, a `formSheet` posed over Stats with an optional `date` param — no param means today, and today is the Europe/Paris day key, never the device's. It leads with « Stat du jour » — « Stat du mardi 12 août » on a past day — the question under that heading and the options under it, each behind its `A` / `B` / `C` quizz letter. It wears the colour of the calendar cell that opens it: `accent` red while today is unanswered, `primary` yellow once it is answered — which also means the surface flips the moment the answer lands. Its texts take that surface's own foreground, since the palette has no muted token for either. Its single detent is `fitToContents`, so the sheet is as tall as the question it carries; that is also why the screen renders a plain column and no scroll view, since a nested scroller gives `fitToContents` nothing to measure. `useDailyQuestion` reads three documents, because the model deliberately does not denormalize the question into the day: `v1_daily_questions/{date}`, the `v1_questions` document it points at, and the current user's answer — whose id *is* their UID, so "already answered" is one `get()` rather than a query. It reads the question only once the day has dropped, matching the rule above, and the four dead ends (`loading` / `unpublished` / `missing` / `error`) are rendered as states of the screen rather than swallowed.
...
- The Stats screen (docs/prd.md §5.2) reads Firestore, and both its daily-question banner and its calendar cells open the day's question. What they cannot open yet is the StatOwrel card of §5.5 — an answered day lands on the same question screen as any other, minus the options it can no longer take.
- The question screen takes an answer — the double tap of docs/prd.md §4.3, `expo-haptics` included — but it does not reward one: validating plays the success animation over the sheet and settles the options into their answered state, where §5.4 wants the sheet to flip to the StatOwrel card of §5.5. The card is the missing half, and the sheet stays dismissable — grabber included — until it exists, rather than pinned open as §5.4 wants: there is nothing yet to pin the user to. A question long enough to overflow the tallest sheet would need a detent array plus `sheetExpandsWhenScrolledToEdge`, not a nested scroll view.
- The app writes the answer and then guesses when the trigger has caught up with it. `answerStore` covers the window on the screens that show the day, and dismissing the sheet drops the answered month from `useStatsData`'s cache and re-reads the profile counters — but nothing waits for the trigger, so a slow one leaves the calendar cell and the counters a visit behind. A listener on the calendar month, or on the profile, is what would close that window.
...
- Six of the PRD's collections exist; only `v1_users/{id}/friends` is still to be modelled — so a handle resolves to an account, but nothing befriends it yet — see `docs/prd.md` §6. The two monthly documents are extra to that list: read models the PRD does not describe, because it describes what the app shows, not what it costs to show it.
