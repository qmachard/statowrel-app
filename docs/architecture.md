...
What a freshly signed-in account meets, once it has a handle and before it sees the Stats screen — `docs/prd.md` §5.6. `apps/app/src/onboarding/` holds it: four slides saying what StatOwrel is, then the demo question popped as a sheet over them.

It is **laid over the app, not routed into it**. `src/App.tsx` renders it beside the navigator the way it renders the username sheet, and behind it: both are blocking, and an account without a handle has nothing behind the carousel yet. Whether it has been through is an `AsyncStorage` entry keyed by UID (`useOnboardingSeen`) — per account, so two accounts on one phone each get it once and signing out does not replay it, but still on the device, since the alternative is a field on `v1_users` the profile rules would have to let a client move. Storage that cannot be read counts as « already seen »: a carousel is not worth risking on every launch. The splash screen is held until that flag resolves as well as the session.

Three things it does that are worth knowing:

- **The demo question is real.** `v1_questions/{DEMO_QUESTION_ID}`, read once by its fixed id, which is what the `demo` rule above exists for. It is posed in the same red sheet as the daily question, with the same double tap and the same result under it, and it **writes nothing**: there is no day and no streak to move, and the rules would refuse the answer anyway. The shares come from the tally the question already carries, which is why `npm run seed-demo-question` puts one there. A question that cannot be read simply ends the carousel a slide early — never a step somebody is stuck on.
- **The permission dialog is primed, not sprung.** The fourth slide says why the 07:00 notification matters, and its button is what registers this phone — `registerDeviceForPush(uid, { ask: true })`, the only caller in the app that passes `ask`. Everything else, the launch registration above all, settles for the permission it finds and never raises a dialog: a refusal is final on both platforms, so the cold prompt at the first signed-in launch was the one thing worth moving.
