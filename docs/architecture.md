...
The friend list of docs/prd.md §5.3 now reads those halves, on the Profile screen (`src/friends/`) — the invitation included, since both halves exist from the moment it is sent. Answering one is still to build: `firestore.rules` allows the accept (`pending` → `accepted`, never by the sender) and the delete that covers refusing, cancelling and removing alike, and nothing calls either yet.
...
- The StatOwrel card of docs/prd.md §5.5 renders what the data carries, and the data carries no picture: there is no **illustration** encart, because an option is a `{ id, label, stat_label }` and nothing else, and no **edition number** (« #142 »), because nothing counts the days since launch. The **share button** of §4.4 and its generated image are not built either, nor are the **friends** of §4.5 under the card — the friendships are read now, but only by the Profile screen's list, never beside an answer. The ultra-rare card takes a violet liseré where §5.5 asks for a holographic ground animated on device tilt.
...
- Friendships are written by the invite callable and read by the Profile screen's friend list, but nothing *answers* an invitation yet — no accept, no refuse, no « Retirer », although `firestore.rules` allows all three. Still missing from §4.1 too: the invitation link, the 6-character code, and the account-deletion pass that would drop the friendships an account leaves behind. The two monthly documents are extra to that list: read models the PRD does not describe, because it describes what the app shows, not what it costs to show it.
...
- Design-system primitives are added as screens need them — `Button`, `TextField`, `Card`, `IconButton`, `Calendar`, `Avatar` so far. No dark-mode theming either (light only).
