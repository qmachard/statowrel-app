...
`packages/models/src/v1_user.ts` — the app user's profile, answering stats and token wallet. The document id is the **Firebase Auth UID**, not a ULID: it is the key `author_id`, `user_id` and friendships point at, and the one `firestore.rules` compares against `request.auth.uid`.
...
| `token_balance` | `number` | the wallet (`docs/prd.md` §4.7). Credited `STREAK_TOKEN_REWARD` (100) every `STREAK_TOKEN_MILESTONE` (10) consecutive days answered on time, in the answer trigger's own transaction; debited by proposing a question |
| `tokens_earned` | `number` | lifetime credited |
| `tokens_spent` | `number` | lifetime debited. Stored rather than inferred from the two above: the day tokens also come from a bought pack, a watched ad or a gift, `earned − balance` stops being what was spent |
