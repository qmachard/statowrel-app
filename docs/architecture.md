...
- The Stats screen (docs/prd.md §5.2) reads Firestore, and both its daily-question banner and its calendar cells open the day's question. What they cannot open yet is the StatOwrel card of §5.5 — an answered day lands on the same read-only question screen as any other.
...
- Nothing on mobile writes an answer, so the answer trigger has never run against real traffic — the calendar's projection is only ever written by the scheduler so far. When that write lands, it also has to drop its month from `useStatsData`'s cache: the trigger rewrites the month behind the app's back.
- No way to rebuild a **user's** calendar month from its answers. The shared month index has its repair path — `dailyQuestions-dailyQuestionsApi` `POST /reindex-months`, plus the check the scheduler now runs on the day it reuses — but `v1_user_calendar_months` has none: if the answer trigger exhausts its retries on one answer, that day is missing from that user's calendar until somebody replays it by hand. The same endpoint, extended to replay `v1_daily_question_answers` for one user and month, is the missing piece.
