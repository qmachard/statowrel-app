...
`src/daily-question/` is the first screen reading Firestore for real (Stats is still on fixtures). The route is `DailyQuestion`, a `formSheet` posed over Stats with an optional `date` param — no param means today, and today is the Europe/Paris day key, never the device's. It leads with « Stat du jour » — « Stat du mardi 12 août » on a past day — the question under that heading and the options under it, each behind its `A` / `B` / `C` quizz letter. It sits on `accent`, the same red as today's calendar cell — the cell that opens it — which is what makes the white and yellow option cards read as the thing to tap; the two muted texts on it take `accent-foreground`, since the palette has no muted token for that surface. Its single detent is `fitToContents`, so the sheet is as tall as the question it carries; that is also why the screen renders a plain column and no scroll view, since a nested scroller gives `fitToContents` nothing to measure. `useDailyQuestion` reads three documents, because the model deliberately does not denormalize the question into the day: `v1_daily_questions/{date}`, the `v1_questions` document it points at, and the current user's answer — whose id *is* their UID, so "already answered" is one `get()` rather than a query. It reads the question only once the day has dropped, matching the rule above, and the four dead ends (`loading` / `unpublished` / `missing` / `error`) are rendered as states of the screen rather than swallowed.
