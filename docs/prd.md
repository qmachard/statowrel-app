...
- Une question déjà tirée ne peut pas ressortir (v1 : jamais de rediffusion).
  **État d'implémentation.** La monnaie existe : le palier verse vraiment ses 100 StatCoins — dans la transaction du trigger de réponse, donc jamais sans la série qui les doit et jamais deux fois —, `firestore.rules` interdit au client de bouger le porte-monnaie *et* de s'ouvrir un profil avec un solde ou une série déjà dedans (un compteur truqué vaut désormais de l'argent), et l'écran Stats porte la carte du §5.2 point 6 : la règle, le solde et le prix. Son bouton reste **inerte** : la dépense n'existe pas encore. Les comptes qui tenaient déjà des séries avant que la monnaie existe sont rattrapés par `npm run backfill-statcoins`, qui rejoue leurs réponses et leur verse ce que chaque palier franchi leur devait. Restent à écrire le callable de proposition et son formulaire, le remboursement des 100 StatCoins quand une question est rejetée, le suivi de ses propres propositions (§5.3) et les notifications à l'auteur — la console de modération, elle, sait déjà lire un `pending`.
