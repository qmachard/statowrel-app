...
| **Carte de récap** | La question du jour, puis **chaque StatOwrel avec son pourcentage** — le `stat_label` de l'option, jamais son libellé : la récap dit dans quelles catégories la journée a rangé les gens —, la sienne en `primary` derrière un tick | La seule surface encadrée de l'écran : bordure noire, ombre dure, remplissage proportionnel derrière chaque libellé |
