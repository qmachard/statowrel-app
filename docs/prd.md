...
- Une question déjà tirée ne peut pas ressortir (v1 : jamais de rediffusion).
  **État d'implémentation.** La monnaie existe : le palier verse vraiment ses 100 StatCoins — dans la transaction du trigger de réponse, donc jamais sans la série qui les doit et jamais deux fois —, `firestore.rules` interdit au client de bouger le porte-monnaie *et* de s'ouvrir un profil avec un solde ou une série déjà dedans (un compteur truqué vaut désormais de l'argent), et l'écran Stats porte la carte du §5.2 point 6 : la règle, le solde et le prix. Son bouton reste **inerte** : la dépense n'existe pas encore. Restent à écrire le callable de proposition et son formulaire, le remboursement des 100 StatCoins quand une question est rejetée, le suivi de ses propres propositions (§5.3) et les notifications à l'auteur — la console de modération, elle, sait déjà lire un `pending`.
...
Trois lignes, et rien de plus. Le titre, « Deviens acteur de StatOwrel » — ce n'est pas un formulaire qu'on annonce, c'est un statut. Sous lui, la règle en sous-titre : « Gagne 100§ pour chaque série de 10 réussie », la seule fois où l'app explique la monnaie. Puis le **portefeuille**, centré dans la carte : le pictogramme de pièces et le solde, « 60§ », en `font-head` à la même taille que le nombre de jours de la carte streak. Les deux chiffres sont sur la même échelle parce que l'un devient l'autre — dix jours de série font cent StatCoins, et c'est tout le propos de l'écran. Pas de jauge : elle mettait sur le solde un poids que la règle porte mieux.
