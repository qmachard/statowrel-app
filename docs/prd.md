...
**Juste après la connexion**, une fois le pseudo choisi (§4.1) et avant que l'écran Stats ne soit vu, quatre slides disent ce qu'est StatOwrel — la question quotidienne, la StatOwrel qu'elle rend, les réponses des amis qu'elle débloque, et pourquoi il vaut mieux être prévenu à 7h — puis une **question démo** vient se poser par-dessus.

- Le carousel se pose **par-dessus toute l'app**, comme la sheet de pseudo (§4.1) : il est piloté par l'état, pas par une route. Il attend que la sheet de pseudo soit passée — les deux sont bloquantes, et un compte sans handle n'a encore rien derrière le carousel.
- Une seule sortie, deux boutons qui y mènent : « Passer » en en-tête, et « C'est parti » au bas du résultat de la démo. Les deux marquent le carousel comme vu **pour ce compte, sur cet appareil**, et il ne se rejoue jamais — un second téléphone le rejoue, ce qui est le côté acceptable du compromis : l'alternative serait un champ sur `v1_users` que les règles devraient laisser un client déplacer.
- La **question démo** est une vraie question de `v1_questions`, portant le statut `demo` (§6) : jamais modérée, jamais tirée au sort, et lisible par n'importe quel compte connecté — ni son auteur ni sa diffusion ne la rendraient lisible autrement. Elle se pose dans la même sheet rouge que la question du jour, avec le même double tap (§4.3) et le même résultat dessous (§5.5), calculé sur les compteurs que la question porte déjà.
- **Elle n'écrit rien.** Aucune réponse n'est enregistrée : il n'y a ni journée, ni streak à faire bouger — et les règles la refuseraient de toute façon, une question démo n'ayant jamais été diffusée. C'est aussi pourquoi les réponses des amis (§4.5) sont le seul bloc absent du résultat : personne n'a répondu à une question qui n'a jamais été posée.
- La **quatrième slide demande la permission de notifier**, et c'est son seul bouton qui la demande ; il enregistre le téléphone dans la foulée (§4.2). La boîte de dialogue système n'est jamais la première chose vue : un refus est définitif sur les deux plateformes (`canAskAgain` retombe à faux), donc on dit d'abord pourquoi. La slide promet **un message par jour, quand la question tombe**, et jamais une heure : « 7h » sur un écran d'accueil se lit comme un réveil, et fait refuser la permission à qui n'est pas du matin. Le refus est le « Ne pas autoriser » du système, pas un second bouton dans l'app.
- C'est **le seul endroit qui demande** : l'enregistrement de lancement, lui, se contente de la permission déjà accordée et ne lève jamais de dialogue. Sans ça la question serait posée à froid au premier démarrage connecté, avant que le carousel ait pu dire à quoi elle sert.
...
- Le carousel se voit une fois, et il n'y a **aucun moyen produit de le revoir** : le bouton qui le remet à zéro n'existe que dans un build de développement (`__DEV__`), sur l'écran Menu, pour que le tester ne demande pas d'effacer les données de l'app entre deux essais.
