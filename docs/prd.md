...
- La **quatrième slide demande la permission de notifier**, et c'est son seul bouton qui la demande — il enregistre le téléphone dans la foulée (§4.2). La boîte de dialogue système n'est jamais la première chose vue : un refus est définitif sur les deux plateformes (`canAskAgain` retombe à faux), donc on dit d'abord pourquoi — un seul message par jour, celui de 7h, sans lequel on découvre la question le soir ou pas du tout. Le refus est le « Ne pas autoriser » du système, pas un second bouton dans l'app.
