...
**État d'implémentation.** La liste existe, sous la carte de récap du §5.5 : avatar, `@handle`, leur StatOwrel (le `stat_label` de l'option choisie, pas son libellé) en puce et l'heure, ceux qui ont répondu comme moi en tête, ceux qui n'ont pas encore répondu en fin de liste. C'est la ligne de la liste d'amis du §5.3, à l'identique — même avatar DiceBear semé sur le handle, même carte découpée par des séparateurs, et **même tape** : elle ouvre l'écran du pote (§5.3), voir ce que quelqu'un a répondu aujourd'hui étant le meilleur moment pour se demander à quel point on répond pareil en général — pour qu'un pote se reconnaisse du même coup d'œil où qu'il apparaisse. Seules les amitiés `accepted` y figurent, et aucune réponse n'est lue tant qu'on n'a pas répondu soi-même — c'est la mécanique, pas une optimisation. Sans aucun pote, la section se réduit à une ligne sur la sheet, sans carte.

### 4.6 Streak

- +1 à chaque journée où l'on a répondu avant minuit.
- **0** dès qu'une journée est manquée. Répondre en retard depuis le calendrier (§4.2) complète la journée dans le calendrier et débloque la carte, mais ne rallume pas le streak cassé — le seul rattrapage qui préserve la série est le **joker du §4.8**, à dépenser sur la question du jour encore ouverte.
- **Tous les 10 jours, la série verse 100 StatFlouzz** (§4.7) — c'est ce que la régularité achète, en plus du chiffre. **Et l'écran de résultat le dit** (§5.5) : une monnaie que personne ne voit arriver ne motive rien, et le palier est justement le moment où l'on vient de gagner le droit de poser sa question.
- Streak visible sur son profil et à côté de son nom dans la liste des amis.
- **Rappel push à 21h si la question du jour n'a pas été répondue et que le streak est en cours.** Fait, et **ciblé** : c'est le ciblage qui répond à la question du §9. Une série est le plus gros actif d'un compte, elle meurt en silence à minuit, et personne ne lui disait ce qu'elle allait perdre. Envoyé à tout le monde ce serait la troisième relance identique de la soirée ; envoyé aux seuls comptes qui ont réellement quelque chose à perdre, c'est un service.
  - **Qui le reçoit** : une seule requête, `streak_last_answered_on == hier`. Une série n'est vivante que si sa dernière réponse date d'hier, et le trigger de réponse écrit ce champ aussi bien pour une réponse que pour un joker — donc à 21h, avoir déjà fait sa journée, avoir déjà posé son joker et traîner un `streak_count` de série morte sont trois exclusions gratuites, sans lire une seule réponse. S'y ajoute un seuil de **7 jours** : une semaine est la plus courte série que quiconque appelle une série, et c'est déjà la frontière que le plan de taggage trace (`streak_bucket`, docs/analytics.md). En dessous, la relance de 18h suffit ; elle part à tout le monde, quel que soit le streak.
  - **Ce qu'il dit** : le titre porte le chiffre (« Ta série de 23 jours va casser »), le corps porte l'échéance et la sortie, et jamais la même chose deux fois. Celui de 18h parle des potes, celui de 21h parle de la série : le repli de 18h a été réécrit pour ça, il ne dit plus « ne perds pas ta série ».
  - **Ce qu'il propose** : un tap ouvre la journée **et** la confirmation du joker (§4.8), donc sauver sa série tient en deux gestes. Uniquement pour un porte-monnaie qui peut payer les 20 StatFlouzz : en dessous, le message invite à répondre et ne cite aucun prix. Le prix n'est d'ailleurs pas dans la notification même quand il est payable — il est dans la confirmation, un écran plus loin, où un chiffre se lit au lieu de s'entrapercevoir.
  - **Comment on le coupe** : il a son propre canal Android (`streak-reminder`), donc le museler laisse intactes la question du matin et les invitations. **iOS n'a pas d'équivalent** : Apple n'offre aucun interrupteur par catégorie qu'une app puisse déclarer, si bien qu'un iPhone ne peut couper que StatOwrel en entier. C'est la limite assumée de cette version ; la lever demande une préférence sur le profil et l'écran de réglages du §5.3, qui n'existe toujours pas.
...
- **Un compte fraîchement créé s'ouvre avec 50 StatFlouzz** — de quoi tenter un joker (§4.8) avant que le premier palier de série ne verse. Rien de plus : proposer une question reste quelque chose à gagner. Cette dotation est comptée à part de `statcoins_earned`, qui garde son sens de « ce que le compte a mérité » (le champ Firestore, lui, reste `statcoins_earned` : renommer la donnée demanderait une migration).
...
- Une question déjà tirée ne peut pas ressortir (v1 : jamais de rediffusion).
  **État d'implémentation.** La boucle complète existe : la monnaie se gagne, se dépense et se rend. Le palier verse vraiment ses 100 StatFlouzz — dans la transaction du trigger de réponse, donc jamais sans la série qui les doit et jamais deux fois — **et il ne les verse plus en silence** : l'écran de résultat annonce la série atteinte, les 100§ et ce qu'ils ouvrent, avec le bouton qui mène ici (§5.5). C'était la seule récompense du produit versée sans être perçue, et c'est aussi ce qui rendait cette carte invisible — un palier vaut exactement une question, et personne n'apprenait qu'il venait d'en gagner une. `questions-proposeQuestion` les dépense : un callable, une transaction qui lit le profil, refuse en dessous du prix, débite `statcoin_balance`, incrémente `statcoins_spent` et écrit la question `pending` du même geste — une question ne peut donc pas exister sans avoir été payée, ni un porte-monnaie se vider sans qu'une question en sorte. C'est la **seule porte** : `firestore.rules` est passé à `allow create: if false` sur `v1_questions`, parce qu'un prix qu'une écriture directe contourne n'est pas un prix. Le remboursement au rejet est un trigger sur la question : il rend le `statcoin_cost` que la question porte — donc rien pour les questions semées, la démo ou celles écrites depuis la console, qui n'ont rien coûté — et il ne le rend qu'une fois, `refunded_at` étant le marqueur qu'il lit dans la transaction même qui crédite (un trigger est délivré au moins une fois, et créditer deux fois un porte-monnaie, c'est de la fausse monnaie). Côté app, la carte du §5.2 point 6 ouvre enfin son formulaire (`src/questions/`), une **modale plein écran** où l'on écrit l'intitulé et de 2 à 6 réponses, listées : « Réponse N » et son bouton de retrait au-dessus de deux champs, un filet entre chacune, « Ajouter une réponse » en pied de liste. Rien n'y est expliqué — les titres nomment les parties, les exemples dans les champs montrent la forme d'une réponse, et le prix est sur le bouton qui le dépense. Les comptes qui tenaient déjà des séries avant que la monnaie existe sont rattrapés par `npm run backfill-statflouzz`, qui rejoue leurs réponses, leur verse ce que chaque palier franchi leur devait **et reconstruit au passage leurs compteurs** — `answers_count`, `streak_count`, `streak_best` : ce sont des valeurs dérivées qu'un trigger incrémente une réponse à la fois, et elles dérivent. **Les quatre notifications à l'auteur existent**, et aucune n'a coûté de fonction : validée, refusée (avec son motif et, quand il y en a eu un, son remboursement), tirée le matin où elle tombe, et le **bilan** le lendemain matin — combien de réponses, et quelle StatOwrel est arrivée en tête. Les deux verdicts sont un second step sous `questions-onQuestionUpdated`, le trigger que le remboursement occupait déjà : il rend la main sur le statut avant toute lecture, donc les milliers de mises à jour d'`answer_counts` que reçoit une question du jour continuent de coûter une invocation et zéro lecture. Écrire le marqueur d'idempotence est lui-même une mise à jour, donc le trigger se rappelle sur sa propre écriture — d'où un marqueur *réclamé* dans une transaction qui relit le statut, jamais posé après coup, et deux marqueurs plutôt qu'un (`approval_notified_at`, `rejection_notified_at`) : un remboursement est définitif, un verdict ne l'est pas. Le tirage et le bilan n'ajoutent rien du tout : le fan-out de 7h résout déjà une notification par utilisateur — c'est ce qui permet au rappel de 18h de donner un compte différent à chacun — donc les deux lignes prennent la place de « La question du jour est tombée » pour les deux comptes concernés, et un auteur ne reçoit jamais deux bannières coup sur coup. Si un même compte a écrit les deux jours, celle du matin gagne. **Seule une vraie proposition a un auteur à prévenir** : c'est `statcoin_cost` qui tranche, puisque `questions-proposeQuestion` est la seule porte qui le pose — une question semée, la démo ou une question écrite depuis la console sont créditées à quelqu'un qui ne les a pas écrites. Un bilan sans aucune réponse ne part pas : « 0 personne a répondu à ta question » est la phrase qui empêche d'en reproposer une. Les trois qui parlent de la proposition elle-même voyagent sur leur propre canal Android (« Mes questions »), pour qu'on puisse les faire taire sans faire taire la question du jour ; celle du matin où la question tombe reste sur le canal de la question du jour, parce que c'est le tirage du jour et que le faire taire coûterait à son auteur le seul matin qu'il ne voulait pas rater. Reste à faire : rien pour §4.7.

### 4.8 Joker

- **Le joker permet de passer la question du jour sans y répondre**, en gardant sa série intacte. Il coûte **20 StatFlouzz** — deux cinquièmes d'un palier, assez pour dépanner une mauvaise journée sans dévaluer la série qu'il protège.
- Il n'est utilisable que sur **la question du jour encore ouverte** — pas sur un jour raté du calendrier ; le rattrapage en retard (§4.2) reste le seul moyen de compléter un jour manqué, et il ne restaure pas la série.
- Il est **illimité** tant que le solde suit : le prix borne l'usage, pas un compteur hebdomadaire ou mensuel.
- Un joker « vaut réponse » pour tout le reste de l'app (**joker complet**) :
  - la série avance comme sur une réponse à temps, et le palier est versé s'il est franchi — et fêté sur l'écran résultat comme sur une réponse (§5.5), la carte de palier étant alors la seule récompense que la sheet a à montrer ;
...
- **Le pseudo est le code.** Il n'y a rien à générer et rien de plus à retenir : `v1_usernames` porte déjà l'unicité d'un handle et sait le résoudre vers un compte (§4.1). Un code à six caractères aurait été une collection de plus, un générateur, une gestion de collisions et un second identifiant à mémoriser, pour la même chose.
- **L'attribution se demande à l'inscription**, sur la sheet de pseudo, dans un champ facultatif « Qui t'a fait venir ? ». C'est le seul moment où elle peut se donner : `referred_by` s'écrit avec le profil et se fige — il n'y a pas d'écran de réglages pour revenir le remplir.
- **C'est la règle Firestore qui porte tout le modèle anti-fraude.** `referred_by` n'est accepté qu'à la **création** du profil, jamais sur une mise à jour : autoriser `null` → valeur plus tard laisserait toute la base installée se déclarer parrainée du jour au lendemain et encaisser à la réponse suivante. Un parrainage est un fait sur la façon dont un compte a commencé.
- **La récompense se verse à la première réponse du filleul**, pas à son inscription : une adresse ne coûte rien à inventer, donc payer un compte créé, c'est payer un compte créé. Un jour répondu est la plus petite chose que fait un vrai utilisateur.
- **La question de démo de l'onboarding (§5.6) ne compte pas.** Elle s'écrit sur le même chemin qu'une vraie réponse, dès la première connexion — un versement qui la laisserait passer paierait à l'inscription en croyant payer à l'engagement.
- **Le parrainage n'ajoute aucune Cloud Function.** Le versement est un traitement de réponse, et le trigger de réponse existe déjà : en créer un second sur le même chemin, ce serait un second trigger Eventarc, une seconde fonction et une seconde invocation à *chaque* réponse de l'app, pour régler une chose qui n'arrive qu'une fois par compte. Chacun garde sa propre transaction, donc le parrainage n'élargit jamais celle qui fait avancer la série. **Et il ne coûte pas non plus une lecture par réponse** : le profil sur lequel il décide est celui que le traitement de réponse vient de lire dans sa propre transaction, passé plutôt que relu — une lecture Firestore à chaque réponse de l'app pour trancher une question qui ne se pose qu'une fois par compte, c'est l'arithmétique qui a fait refuser le second trigger.
- **Le parrain gagne 30 StatFlouzz, le filleul 10.** Deux côtés, parce qu'un parrainage à sens unique demande au filleul de finir une inscription au bénéfice de quelqu'un d'autre — mais **pas moitié-moitié**, parce que les deux ne font pas la même chose : amener quelqu'un est un travail (un message envoyé, un pseudo transmis, une inscription accompagnée), taper le nom d'un parrain est un champ rempli une fois. La récompense doit être du côté de l'effort, sinon le geste ne se déclenche pas : à dix chacun, amener un humain valait un cinquième de question quand dix jours de série en valaient une entière — répondre était dix fois plus rentable que recruter. Les dix du filleul restent petits exprès : c'est un accueil, pas une prime, et plus gros ça met un prix sur un compte jetable.
- **Un compte n'est payé que pour ses 20 premiers filleuls.** Pas un modèle de fraude, un plafond : ce qui rend le farm non rentable, c'est le prix d'un faux parrainage (une adresse, une inscription, un vrai jour répondu) face à ce qu'il rapporte, vingt StatFlouzz répartis sur deux comptes. Au-delà du plafond, le filleul touche quand même son bonus — il n'y est pour rien.
- **Le filleul est ajouté en ami du parrain**, en invitation `pending` envoyée en son nom à la création du profil. Quelqu'un qui arrive par un lien n'a pas à repartir chercher la personne qui le lui a envoyé, et c'est aussi comme ça que le parrain apprend un **nom** : la notification d'invitation ordinaire (§4.1). Un refus ne coûte rien — la récompense ne lit jamais l'amitié : refuser dit qu'on ne veut pas de quelqu'un dans sa liste, pas qu'il ne vient pas de là.
- **Les deux côtés sont notifiés au versement.** Le parrain apprend *qui* — c'est toute la réponse à « comment je sais que celui-là vient de moi » — et le filleul apprend *pourquoi* son solde a bougé. Un porte-monnaie qui grossit de dix sans une phrase attachée est la seule chose qu'une monnaie ne peut pas se permettre (même raison que la ligne de remboursement du §4.7).
- **Il n'y a pas d'écran « Mes filleuls ».** Amener quelqu'un est quelque chose qu'on *fait*, pas quelque chose qu'on revient lire : une liste de filleuls serait un écran ouvert une fois. Le parrain apprend qui est arrivé par la notification du versement, qui le nomme, et par l'invitation d'ami que le filleul lui envoie à l'inscription. Ce qui reste côté données est un **reçu** (`v1_user_referrals`, écrit par le backend seul, lu par rien aujourd'hui) : un versement qui déplace deux porte-monnaie doit laisser une trace par parrainage, `referrals_count` ne sachant pas dire *qui*, et l'historique d'une monnaie ne se reconstruit pas après coup.
- **Le reçu est dénormalisé côté parrain** (`v1_users/{uid}/v1_user_referrals`) plutôt que lu par requête sur `referred_by` : les règles ne savent pas restreindre un `list` à un filtre, donc cette requête aurait été un annuaire déguisé — ce que le §4.1 interdit. C'est aussi ce qui fera de la liste, le jour où elle vaudra un écran, une seule lecture de collection.
- **Le partage vit dans la sheet d'invitation** (§4.1), sous le champ qui prend un pseudo : ce champ ne joint que quelqu'un qui a déjà l'app, et le lien est l'autre moitié de la même question. Une seule porte, à l'endroit où l'on vient déjà chercher un pote.
- **Le partage envoie un lien**, `https://statowrel-app.web.app/i/lou`, et le pseudo est *dedans*. Un lien `statowrel://` n'ouvrait rien sur un téléphone sans l'app — c'est-à-dire tous ceux à qui on l'envoie — donc le message envoyait l'adresse du site et le handle à recopier. Un lien `https` ouvre l'app quand elle est là (lien universel iOS, App Link Android, tous deux limités à `/i/*` : les pages légales doivent rester au navigateur, ce sont elles que les fiches des stores pointent) et la page d'atterrissage quand elle ne l'est pas.
- **Le pseudo est gardé jusqu'à l'inscription.** Un lien est tapé par quelqu'un qui, par définition, n'a pas encore de compte, et `referred_by` ne s'accepte qu'à la création du profil : le handle attend donc sur le téléphone (`AsyncStorage`, même motif que la réponse de démo du §5.6) et pré-remplit « Qui t'a fait venir ? ». Jamais par-dessus ce qui a été tapé — c'est une suggestion, pas une dictée. Un lien ouvert par un compte qui existe déjà ne peut plus être attribué : il ouvre la sheet d'invitation avec le pseudo, ce qui est la seule chose utile qui reste.
- **Il n'y a pas d'attribution différée gratuite**, et c'est ce qui dessine la page d'atterrissage. Firebase Dynamic Links est mort le 25 août 2025, et aucun des deux stores ne transmet à une installation neuve la page d'où elle vient : le détour par le store perd tout. Donc le lien pré-remplit quand l'app est déjà là, et sinon la page rend le pseudo **lisible** — en grand, dans son propre cadre, copiable d'un geste. Elle ne colle rien : l'app ne *lit* jamais le presse-papier, une lecture programmatique déclenchant sur iOS 16+ la bannière système de collage, payée par toutes les inscriptions pour servir celles qui viennent d'un lien.
- **Reste à faire** : la vérification du domaine par les deux plateformes, qui n'est pas du code. L'`apple-app-site-association` a besoin du Team ID Apple et l'`assetlinks.json` des empreintes SHA-256 des deux clés de signature Android (celle d'upload EAS *et* celle de Play, qui re-signe l'AAB) ; le dépôt les porte en `REPLACE_WITH_…` et `npm run check-app-links` refuse le déploiement tant qu'elles y sont. Les deux fichiers doivent être **en ligne avant** qu'un build ne soit installé : iOS ne relit pas l'association après coup.
...
**1. En-tête.** Le bouton d'invitation à gauche ; à droite le solde de StatFlouzz (puce `muted`, sans bordure ni ombre — la même surface recessée qu'une case de calendrier hors mois — picto pièce + montant en `font-head`) puis le bouton menu. Le portefeuille reste visible pendant qu'on scrolle l'écran où il se dépense, et sa surface à plat le pose comme une lecture plutôt que comme une troisième action à côté des deux boutons.
...
**6. Proposer une question.** Sous le calendrier, un **bouton nu** pleine largeur qui porte le §4.7 : « Poser une question » puis le prix, « 100§ ». Le solde est annoncé dans l'en-tête (§5.2 point 1), la règle de gain vit là où on la comprend — sur la carte streak et ses paliers — donc cette rangée n'a rien à répéter : elle est le geste, pas la leçon. Le prix prend la fente `trailingLabel` du bouton (`font-sans`, un cran plus petit) parce qu'il qualifie l'action sans en être le nom. Sous le prix le bouton est **grisé** (variante `outline`, opacité 0.5) mais reste **tapable** : au tap une alerte native explique pourquoi rien ne s'ouvre — « Poser une question coûte 100§. Il te manque X§. Gagne 100§ tous les 10 jours de série. » — plutôt que de laisser deviner si le bouton est cassé ou le portefeuille vide. Au-dessus du prix il passe en `primary` et ouvre le formulaire du §4.7, en modale plein écran, qui se referme sur ce que la proposition a coûté — « Il te reste 40§ ».
...
- **Mes amis** : liste avatar + `@handle` + streak, avec l'action « Retirer ». En tête de liste, un bouton plein `primary` « Inviter un pote » (partage du lien + code à 6 caractères, §4.1). Si la liste est vide, l'état vide occupe la place de la liste : « Sans potes, StatOwrel c'est juste des chiffres. »
  **Implémenté**, à deux écarts près : le **streak** n'est pas sur la ligne — il n'est pas recopié sur l'amitié, et une valeur qui bouge tous les jours serait périmée à l'affichage ; et les **invitations en attente** sont des lignes de cette même liste, placées en tête, parce que les deux moitiés d'une amitié existent dès l'envoi — les cacher perdrait l'invitation reçue. La liste est une carte découpée en lignes par des séparateurs ; elle ne porte plus de titre à elle, étant l'un des deux panneaux d'un **sélecteur d'onglets** — « Mes potes » / « Mes questions » — qui remplace l'empilement des deux listes : empilées, elles faisaient de l'écran un scroll sans fin qui enterrait les réglages sous deux listes qui ne font que grandir. Inviter se demande à deux endroits : une icône **en haut à droite de l'écran**, à côté du retour, atteignable quel que soit l'onglet — c'est la chose qu'on vient faire ici — et un bouton **pleine largeur sous la liste**, qui la dit en toutes lettres là où il n'y a plus rien à partager la ligne ; l'onglet des questions se termine de la même façon, sur son propre « Poser une question », une invitation porte ses deux réponses en boutons **sous** la mention « T’a envoyé une invitation. » — « Accepter » et « Refuser » sur une invitation reçue, « Annuler » sur une invitation envoyée — la réponse sous ce qu’elle répond ; les invitations reçues sont reprises en tête de l’écran Stats (§5.2). La ligne acceptée, elle, ne porte plus de menu mais un **chevron** : elle mène quelque part, et « Retirer ce pote » a suivi sur l’écran du pote — une ligne qui s’ouvre ne doit pas aussi tenir un menu, et retirer quelqu’un a sa place sur la page qui parle de lui. Le streak est bien là, mais **une tape plus loin** : une ligne acceptée ouvre l’écran du pote décrit juste en dessous, ce qui donne au streak un endroit où être frais plutôt qu’une copie périmée sur l’amitié.
- **Écran d’un pote** : ouvert en tapant sa ligne dans la liste (ou sa ligne dans les réponses du jour, §4.5), il porte deux choses — sa série, son record et ses jours répondus (§4.6), lus sur son propre profil, et la **compatibilité** avec lui. « Retirer ce pote » est dans le menu en haut à droite, la seule action de la page.
  **Implémenté.** La compatibilité est un **taux d’accord brut** : les jours où l’on a tous les deux répondu à la même question, et parmi eux la part où l’on a choisi la même option. Les jokers (§4.8) et la démo d’onboarding (§5.6) en sont exclus — le premier n’a rien choisi, la seconde n’a jamais été une journée et tout le monde y répond. Pas de correction du hasard par le nombre d’options : le chiffre est fait pour être dit à voix haute, et un score que personne ne sait relire est un score que personne ne cite. Sous cinq jours en commun, rien n’est affiché — un seul jour identique fait 100 %, ce qui est une coïncidence déguisée en statistique — et la carte décompte ce qui manque à la place. Elle est **rose** (`notification`), la seule surface de l’app à l’être : le jaune dit déjà « ta réponse » sur la sheet du jour et « répondu » sur le calendrier, et ce chiffre-là n’appartient ni à l’un ni à l’autre — il appartient aux deux. Elle se lit de haut en bas comme une seule phrase : **les deux avatars** qui se chevauchent, « Vous êtes compatibles à », le pourcentage en gros, puis **la ligne que ce chiffre vaut** — « Même cerveau, deux téléphones. », « Pile ou face. », « Tout vous oppose. Et vous êtes potes quand même. » selon la tranche. C’est la punchline qui fait revenir sur la carte, le pourcentage seul n’étant qu’un fait. Le nombre de jours sur lequel le score est calculé n’est **pas** affiché : c’est un titre, et une carte qui montre son arithmétique invite à discuter l’arithmétique plutôt qu’à citer le chiffre — les cinq jours minimum sont ce qui la tient honnête à la place. Les deux visages restent affichés dans tous les états, chargement et échec compris : ils disent de qui parle la carte, ils ne font pas partie du résultat.
  **Ça passe par un callable, `friends-getFriendCompatibility`, et rien d’autre.** Comparer deux historiques est exactement la requête que `firestore.rules` interdit à tout client : le groupe de collections `v1_daily_question_answers` est borné à ses propres réponses, pour qu’ouvrir la journée d’un pote ne devienne jamais lire son historique. Le calcul se fait donc côté admin, et ce qui en sort est trois nombres. Le callable vérifie l’amitié (`accepted`) avant de lire quoi que ce soit : sans ça, n’importe quel compte connaissant un UID pourrait demander à quel point il ressemble à n’importe qui.
  **Deux caches, un appel par pote et par jour.** Le score ne bouge que quand l’un des deux répond, donc il est recalculé au plus une fois par paire et par jour parisien, dans `v1_friend_compatibilities/{pair_id}` — document par **paire**, identifiant fait des deux UID triés, lisible par les deux et par personne d’autre. Côté téléphone, `compatibilityCache` garde le dernier score par pote : tant qu’il porte la clé du jour, revenir sur le profil ne repart même pas au réseau. Rien là-dedans n’est une source — tout se recalcule depuis les calendriers —, donc un cache perdu coûte un recalcul et aucune donnée.

  **Et le recalcul est un diff.** Il lit `v1_user_calendar_months` et non les réponses : le calendrier porte déjà l’`option_id` de chaque jour répondu, un document par mois et par personne, donc un an d’historique se lit en douze documents au lieu de trois cent soixante-cinq. Le document de paire garde en plus le décompte **découpé par mois** et le curseur `synced_at` : une passe ne relit que les mois qui ont bougé depuis, et ce qu’elle recalcule, elle le recalcule en entier — donc deux passes qui se chevauchent écrivent les deux mêmes nombres. À douze mois d’historique et dix potes consultés dans la journée, le comparateur coûtait ~7 300 lectures ; il en coûte ~40, et ce nombre ne grandit plus avec l’âge des comptes. Les deux exclusions sortent gratuitement de la source : un joker est projeté dans `jokers` et jamais dans `days`, la démo d’onboarding dans aucun mois.
...
| **Carte de récap** | La question du jour, puis **chaque option avec son pourcentage** — son libellé, pas sa StatOwrel : la récap rappelle ce qui a été répondu —, la sienne en `primary` derrière un tick | La surface encadrée de l'écran — la seule, hors la carte de palier les jours où il y en a une : bordure noire, ombre dure, remplissage proportionnel derrière chaque libellé |
| **Palier de série** | Uniquement le jour où la réponse franchit un palier (§4.6) : « Série de 10 jours », **+100§**, et la ligne qui compte — de quoi poser sa question — avec le bouton qui ouvre le formulaire (§4.7) | Carte `primary` jaune, **sous la phrase** : la StatOwrel reste la récompense de l'écran, le palier est ce que la journée vient d'acheter |
...
- Ce résultat est **rejouable à volonté** : tap sur un jour répondu dans le calendrier (§5.2) le rouvre à l'identique, avec les stats à jour et les réponses des amis arrivées depuis. **La carte de palier est la seule exception** : elle appartient au moment où la série franchit le palier, pas au jour qui l'a franchi, donc elle ne réapparaît jamais à la réouverture. Un rattrapage tardif (§4.2) ne fait pas avancer la série et ne fête donc rien.
- **Rien n'est annoncé qui n'ait été versé.** Le versement est décidé côté serveur, dans la transaction du trigger de réponse, alors que l'écran s'affiche dès que la réponse est écrite. L'app ne prédit donc pas la décision : elle fige `streak_count` et `statcoins_earned` juste avant l'écriture, et n'affiche la carte que quand l'abonnement au profil prouve que la série a franchi un palier **et** que le compte a bien été crédité d'au moins ce que ce palier vaut. Elle arrive donc un battement après la StatOwrel, sans jamais la retarder. **Le prix de cette règle est assumé** : une réponse écrite hors ligne ne se fait créditer qu'à la reconnexion, souvent après que la sheet a été fermée — le palier est alors versé sans être fêté. Une fête manquée coûte moins cher qu'une promesse non tenue.
...
| Collection | Contenu |
|---|---|
| `v1_users` | `username`, `photo_url`, `created_at`, `updated_at`, `email`, `auth_providers[]` (`password` / `google.com` / `facebook.com` / `apple.com`), `streak_count`, `streak_best` (meilleur streak, §5.3), `streak_last_answered_on`, `statcoin_balance` / `statcoins_earned` / `statcoins_spent` (le porte-monnaie, §4.7), `referred_by` (UID du parrain, écrit à la création et figé — §4.9), `referrals_count` et `referral_rewarded_at` (les deux compteurs backend du parrainage) |
...
| `v1_users/{id}/v1_user_referrals` | une entrée par filleul, **id de document = l'UID du filleul** : `user_id`, `referred_user_id`, `referred_username` (recopié, même raison que `friend_username`), `statcoins_earned` (ce que *ce* parrainage a payé, pas ce que la constante paierait aujourd'hui), `created_at` (l'inscription du filleul), `rewarded_at` (`null` tant que le filleul n'a pas répondu). Écrite par le backend seul — une ligne ici est un reçu, et un reçu que son bénéficiaire écrit n'en est pas un (§4.9) |
...
- ~~Le rappel de 21h est-il perçu comme utile ou comme du harcèlement ?~~ **Tranché : utile, à condition d'être ciblé** (§4.6). Un rappel est utile quand il nomme ce que la personne va perdre et lui propose de le sauver ; il devient du harcèlement quand il part à tout le monde et se contente de répéter. D'où les trois règles qui le tiennent : seuls les comptes dont la série est vivante et assez longue le reçoivent, il ne redit rien de celui de 18h, et il porte le joker plutôt qu'une relance. Reste à mesurer : le taux d'ouverture et la part de séries sauvées par joker depuis cette notification.
