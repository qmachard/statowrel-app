...
- **Reste à faire** : les **liens universels** (`statowrel.app/i/lou` ouvrant l'app, ou le store puis l'app) et la page d'atterrissage web qui va avec. Sans eux, un lien `statowrel://` n'ouvre rien sur un téléphone qui n'a pas l'app — c'est-à-dire tous ceux à qui on l'envoie — donc le partage envoie l'adresse du site et le handle à recopier, et l'attribution se tape. Avec eux, elle se pré-remplira.
