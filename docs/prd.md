...
- Une question déjà tirée ne peut pas ressortir (v1 : jamais de rediffusion).
  **État d'implémentation.** La monnaie existe : le palier verse vraiment ses 100 jetons — dans la transaction du trigger de réponse, donc jamais sans la série qui les doit et jamais deux fois —, `firestore.rules` interdit au client de bouger le porte-monnaie *et* de s'ouvrir un profil avec un solde ou une série déjà dedans (un compteur truqué vaut désormais de l'argent), et l'écran Stats porte la carte du §5.2 point 6 : le solde, sa jauge vers les 100 jetons et la règle qui la fait monter. Son bouton reste **inerte** : la dépense n'existe pas encore. Restent à écrire le callable de proposition et son formulaire, le remboursement du jeton quand une question est rejetée, le suivi de ses propres propositions (§5.3) et les notifications à l'auteur — la console de modération, elle, sait déjà lire un `pending`.
...
**6. Proposer une question.** Sous le calendrier, une **carte** qui porte toute l'économie du §4.7 — et pas seulement un bouton. Un bouton nu suffisait tant que la condition était un palier unique de 30 jours : une phrase disait une porte qui s'ouvre une fois et reste ouverte. Une monnaie ne tient pas en une phrase. Il y a un prix, un solde, la distance entre les deux et la façon de la combler, et un bouton qui ne sait dire que ce qui manque laisse deviner d'où viennent les jetons.

De haut en bas : le titre « Ta question » et sous lui ce qu'on y gagne — « Une fois validée, tout le monde y répond à 7h. » ; puis le solde en `font-head` face à sa cible en petites capitales grises (« 100 pour poser »), et sous eux la **jauge** — piste `muted` bordée de noir, coins pleinement arrondis, remplissage `primary`, plafonnée à 100% pour qu'un solde de trois questions la remplisse sans la déborder ; puis, sur une ligne à part avec son pictogramme de pièces, la règle : « Tous les 10 jours de série, tu gagnes 100 jetons. » Le pied de carte, `muted` et bordé haut comme tout `CardFooter`, ne porte que le bouton « Proposer une question », pleine largeur.

La règle reste affichée une fois le prix couvert : ce n'est pas une condition qu'on poursuit, c'est le fonctionnement de l'économie, aussi vrai avec 500 jetons en poche qu'avec zéro. Sous le prix le bouton est `outline` et cadenassé, au-dessus il passe en `primary` — la carte, elle, garde sa surface `card` dans les deux cas : la ligne de stats a déjà sa carte `primary`, deux sur un écran se disputeraient l'œil. **Le solde ne figure pas dans la ligne de stats** : un nombre ne se lit que contre le prix vers lequel il monte, et le dire deux fois sur un écran, dans deux cadrages, se lit comme deux choses différentes. Le formulaire derrière n'existe pas encore, et tant qu'il n'existe pas le bouton reste inerte même pour qui peut se le payer.
