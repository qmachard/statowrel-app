...
**3. Bloc stats.** Deux colonnes de largeur égale. À gauche, la carte streak sur toute la hauteur du bloc : `primary` bordée, ombre `lg`, la flamme et le libellé en haut, le nombre de jours en `font-head` puis « jours d'affilée », le tout centré verticalement. Quand le streak est à 0, elle passe en `muted` avec « Réponds aujourd'hui pour repartir ». À droite, deux tuiles `card` empilées, bordure noire, ombre dure : le meilleur streak jamais atteint (`streak_best`) et le nombre total de jours répondus depuis l'inscription. Elles sont sur la même échelle typographique que le streak — il tient sa place par sa surface et sa couleur, pas par la taille de son chiffre — et remettent le streak courant en perspective sans lui voler la vedette.
