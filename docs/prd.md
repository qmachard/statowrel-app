...
- **La question de démo de l'onboarding (§5.6) ne compte pas.** Elle s'écrit sur le même chemin qu'une vraie réponse, dès la première connexion — un trigger qui la laisserait passer paierait à l'inscription en croyant payer à l'engagement.
- **Les deux côtés gagnent 10 StatFlouzz.** Un parrainage à sens unique demande au filleul de finir une inscription au bénéfice de quelqu'un d'autre. Dix chacun reste modeste — un cinquième d'une question, la moitié d'un joker — pour qu'une mécanique qui suit un carnet d'adresses ne dilue pas la monnaie.
...
- **« Mes filleuls » vit sur le Menu** (§5.3), sous « Mes questions ». Une ligne existe **dès l'inscription du filleul**, pas au versement : le versement attend une première réponse qui peut être à plusieurs jours, et une carte restée vide dirait que l'attribution n'a pas marché. Une ligne non réglée porte « en attente ».
- **La liste est dénormalisée côté parrain** (`v1_users/{uid}/v1_user_referrals`) plutôt que lue par requête sur `referred_by` : les règles ne savent pas restreindre un `list` à un filtre, donc cette requête aurait été un annuaire déguisé — ce que le §4.1 interdit.
