...
#
# Usage: setup-templates.sh "feature-name" "task description" [flags...]
...
render_template() {
    ...
    local teams_status="⏭ Skip"
    [[ "$TEAMS_MODE" == "true" ]] && teams_status="⏸ Pending"
    ...
    sed -e "s|{{task_id}}|${TASK_ID}|g" \
        ...
        -e "s|{{teams_status}}|${teams_status}|g" \
...
if [[ "$TASKS_MODE" == "true" ]] || [[ "$TEAMS_MODE" == "true" ]]; then
