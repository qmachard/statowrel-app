...
- **Design system**: neobrutalism (reference: [neoflux](https://neobrutalism.com/preview/templates/neoflux)) — bold colors, thick borders, hard offset shadows, and slightly rounded corners (`rounded` = 4px — not square). The palette **and** the radius scale live in `apps/app/src/design/tokens.js` (read by both `tailwind.config.js` and the navigation theme); the rest of the tokens (`font-head`/`font-sans`, `borderWidth`, `boxShadow`) live in `apps/app/tailwind.config.js`; fonts (Archivo Black, Space Grotesk) load via `expo-font` + `@expo-google-fonts/*`. The neobrutalism.com `shadcn` registry itself is web-only (Radix/Base UI need a DOM) — it does not apply to this React Native app; reusable component primitives are hand-built against these tokens (`apps/app/src/components/`).
