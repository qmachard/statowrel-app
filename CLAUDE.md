...
- **Design system**: neobrutalism / sticker — bold flat colors, thick black borders, hard offset shadows, `radius: 0`. Four inks only (cream, golden yellow, bubblegum pink, black), in `apps/app/src/theme/colors.js`, read by both `tailwind.config.js` and the app runtime. The rest of the tokens (`font-head`/`font-sans`, `borderRadius`, `borderWidth`, `boxShadow`) live in `apps/app/tailwind.config.js`; fonts (Archivo Black, Space Grotesk) load via `expo-font` + `@expo-google-fonts/*`. Pictograms are hand-built SVG stickers (`apps/app/src/components/icons/`) — never an icon library, never emoji. The neobrutalism.com `shadcn` registry itself is web-only (Radix/Base UI need a DOM) — it does not apply to this React Native app; reusable component primitives are hand-built against these tokens as a later step.
