 ...
 * than one joker.
 *
 * **Called from `daily-questions`' own answer trigger**, not from one of this
 ...
 * group, so a payout that did not exclude it would land at sign-up while
 * believing it landed at engagement. The trigger drops it on the question id,
 * before this step is reached and before anything is read.
 *
...
export const payReferralReward = async (userId: string, user: UserData | null): Promise<void> => {
  ...
  const settled = await runTransaction<SettledReferral | null>(async (transaction) => {
    ...
    if (rowSnap.exists) {
      ...
      // anyway. Writing it now is what keeps the receipt trail complete;
