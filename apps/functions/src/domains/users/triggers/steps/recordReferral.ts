 ...
 *
 * 1. **It opens the referral's receipt** on the sponsor's side,
 *    `rewarded_at: null` until the payout lands. Nothing renders it today —
 *    the sponsor hears about a referral through the push the payout sends —
 *    but a payout that moves two wallets has to leave a per-referral trace, and
 *    opening it at sign-up is what makes an attribution that never converted
 *    part of that record. See `v1_user_referral.ts`.
...
export const recordReferral = async (userId: string, user: UserData): Promise<void> => {
  ...
  if (existing === null) {
    const row: UserReferralData = {
      ...
      // `referrals/helpers/payReferralReward.ts`.
      statcoins_earned: 0,
