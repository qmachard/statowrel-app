...
export const deleteAccount = onCall<unknown, Promise<DeleteAccountResult>>(
  ...
  async (request) => {
    ...
    const [ profile, answers, months, devices, friendships, referrals ] = await Promise.all([
      ...
      // Who this account brought in (docs/prd.md §4.9). Its own receipts, so
      // they go with it — the newcomers keep their accounts and their wallets, and
    ...
    // The other side of this account's own referral: the receipt sitting under
    // its sponsor. Left standing it would name an account that no longer
    // exists, under a handle about to be free for somebody else to take.
    //
