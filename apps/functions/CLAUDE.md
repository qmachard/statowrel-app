...
A trigger on a hot collection is worth a word too. `v1_questions` takes an `answer_counts` increment on every answer given in the app, and a Firestore trigger cannot be filtered on a field, so `questions-onQuestionUpdated` runs on that whole path: its step returns on the status before it reads anything, which is what keeps the common case at one invocation and no Firestore read. The same arithmetic runs through `onDailyQuestionAnswerCreated`, where the cost is a read rather than an invocation: `payReferralReward` settles something that happens once per account, so `onAnswerCreated` hands it the profile it already read in its own transaction instead of letting it fetch the same document a second time on every answer given in the app.
...
`--sample` is the flag to reach for while iterating on the design: five canned days covering both things the card varies on — 2 to 6 options, the bounds `QUESTION_MIN_OPTIONS` and `QUESTION_MAX_OPTIONS` set, with a long question and a long StatOwrel; and a dominant share on every rung of the hedge ladder, « peut-être » at 24 % through « forcément » at 92 %. That is the layout's worst case and the copy's whole range in one run, neither of which a real database offers on the day you need it. It touches no project and needs no credentials.
