...
- `src/stats/` — the Stats screen (docs/prd.md §5.2), the app's root. `screens/StatsScreen.tsx` composes `components/` (`StatsHeader`, `DailyQuestionBanner`, `StatsStrip` holding `StreakCard` and the two `StatTile`s, `StatsCalendar` + its `CalendarDay` and `Hatch`), `helpers/calendarState.ts` derives the four day states and `helpers/streak.ts` the streak actually worth showing, and `data/useStatsData.ts` is the whole data source.
  ...
  **The calendar costs two reads a month, and that is the point.** The streak, the record and the answered-days total come from the `v1_users/{uid}` document `AuthContext` already holds — free. The displayed month is one read on `v1_daily_question_months/{YYYY-MM}` (which days had a question) plus one on `v1_users/{uid}/v1_user_calendar_months/{YYYY-MM}` (which days this user answered, with the `stat_label` the cell renders). Never query `v1_daily_question_answers` for a month: that is one read per answered day plus two joins each, which is exactly what those two documents exist to avoid. Months already visited are kept in a per-mount ref, so a chevron back costs nothing.

  The daily-question banner rides along on the same two documents — its label is copied onto the month index and "already answered" is a day of the user's month — so it costs nothing either. It reads the *current* month rather than the displayed one, which is why `useStatsData` returns `todayQuestion` and `answeredToday` beside `calendar`: browsing back to March must not make today's question vanish from the top of the screen.

  When the answer flow lands (docs/prd.md §5.4), answering has to drop its month from that cache — the backend rewrites the month behind the app's back.
