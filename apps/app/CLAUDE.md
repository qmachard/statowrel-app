...
- `src/design/tokens.js` — the neobrutalism palette and corner-radius scale, in CommonJS so `tailwind.config.js` can `require()` it while TypeScript imports the same values for the navigator chrome. One source, no drift.
...
- `tailwind.config.js` — Nativewind preset + neobrutalism theme tokens: the palette and `borderRadius` scale from `src/design/tokens.js`, plus `font-head`/`font-sans`, thick `borderWidth`, hard offset `boxShadow` scale.
