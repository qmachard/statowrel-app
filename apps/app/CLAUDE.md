...
- Icons are hand-built stickers in `src/components/icons/shapes.tsx` — never emoji, and no icon library: an off-the-shelf set is drawn as thin uniform strokes, which disappears next to 2px borders and hard offset shadows. Each shape takes a `fill` from `@/theme/colors`. Wrap it in `<Sticker>` for the offset shadow copy, or in `<StickerButton>` for the round disc button.
- The shapes are composed of plain `View`s — rounded corners, rotations, borders — **not SVG**. `react-native-svg` is a native module: pulling it in would force everyone to rebuild the dev client for a handful of geometric shapes. Keep it that way, and keep any new dependency JS-only unless a native rebuild is already on the table. The outline is a black copy of the shape underneath, not a `borderWidth` — a shape made of several overlapping views would show its internal seams wherever two bordered pieces meet.
