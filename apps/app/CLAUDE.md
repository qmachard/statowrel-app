...
- `src/stats/` — the Stats screen (docs/prd.md §5.2), the app's root. `screens/StatsScreen.tsx` composes `components/` (`StatsHeader`, `DailyQuestionBanner`, `StatsStrip` holding `StreakCard` and the two `StatTile`s, `StatsCalendar` + its `CalendarDay` and `Hatch`), `helpers/calendarState.ts` derives the four day states and `helpers/streak.ts` the streak actually worth showing, and `data/useStatsData.ts` is the whole data source.
  ...
  **The calendar costs two reads a month, and that is the point.** The streak, the record and the answered-days total come from the `v1_users/{uid}` document `AuthContext` already holds — free, and re-read on focus only after an answer has moved its counters. The displayed month is one read on `v1_daily_question_months/{YYYY-MM}` (which days had a question) plus one on `v1_users/{uid}/v1_user_calendar_months/{YYYY-MM}` (which days this user answered, with the `stat_label` the cell renders). Never query `v1_daily_question_answers` for a month: that is one read per answered day plus two joins each, which is exactly what those two documents exist to avoid. Months already visited are kept in a per-mount ref, so a chevron back costs nothing — and answering drops its month from that cache through `invalidateCalendarMonth`, since the answer trigger rewrites it behind the app's back.
...
- `src/daily-question/` — one day's question (docs/prd.md §5.4), the sheet that lands on top of Stats. `screens/DailyQuestionScreen.tsx` owns the **double tap** of §4.3 — first tap selects, second one validates behind a 150 ms guard — `components/QuestionOption.tsx` renders the three states it drives, and `data/` is the day: `useDailyQuestion.ts` reads it, `submitAnswer.ts` writes the answer at `v1_daily_questions/{date}/v1_daily_question_answers/{uid}` (**document id = the Firebase Auth UID**, which is what makes one answer per person per day a property of the data), and `answerStore.ts` holds what this session has written so the sheet and the Stats banner under it flip together. The write is the app's only one under a daily question: `answer_counts`, the calendar month and the streak all belong to the answer trigger.
- `src/components/` — neobrutalism primitives (`Button`, `TextField`, `Card`, `Calendar`, `BottomSheet`) and `animations/` (the Lottie presets `SuccessCheck` / `SuccessCircle` over the shared `Animation`). `Card` and `Calendar` are React Native ports of the [neobrutalism.com](https://neobrutalism.com/docs/components/card) components: same anatomy, minus the `has-*` variants and the DOM-only `react-day-picker`. An icon-only button is `Button` at an `icon*` size, not a separate component.
- `src/components/animations/` — the Lottie feedback animations, on `lottie-react-native` (a **native** module: adding it means rebuilding the dev client). `sources.ts` is the registry of the `assets/lottie/` compositions, `Animation.tsx` the sized wrapper reading it, and `SuccessCircle` / `SuccessCheck` the presets that pick a composition. A new animation is a JSON in `assets/lottie/` plus a line in the registry.
