...
- `src/menu/` — what opens from the header's menu button (docs/prd.md §5.1), stacked over Stats with its own back button. `screens/MenuScreen.tsx` holds the account today — handle, e-mail, sign-in providers, sign out; the settings, the friends and the proposed questions land here as they come.
