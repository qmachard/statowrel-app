...
- Anything a backend trigger writes is **subscribed to**, not read: `v1_users/{uid}` and the current calendar month both move after the app writes an answer, and a `getDoc` would have to guess when. Read once instead when the document is settled — a past calendar month, a question that has been broadcast. A hook that resets state at the top of an effect to switch keys trips `react-hooks`; carry the key on the state and derive what is current, as `useDailyQuestion` does.
