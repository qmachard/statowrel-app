...
- `app/` — [Expo Router](https://docs.expo.dev/router/introduction/) file-based routes. `_layout.tsx` is the root layout (`SafeAreaProvider` + `AuthProvider`, imports `global.css`, holds the splash screen until the persisted session resolves). `app/(auth)/` holds `sign-in` and `sign-up`; `app/verify-email.tsx` gates unverified email accounts; `app/index.tsx` is protected. Add screens as `app/<route>.tsx` / `app/<route>/_layout.tsx`.
- `src/auth/` — everything authentication. `AuthContext.tsx` (`useAuth()`: `user`, `profile`, `initializing`, `requiresEmailVerification`, `refreshUser`), `providers.ts` (Google / Apple / email primitives + `signOut`), `profile.ts` (upserts `v1_users/{uid}` — **id = Firebase Auth UID**), `profileHints.ts`, `schemas.ts` (zod), `errors.ts` (French messages for `auth/*` codes), `SocialSignInButtons.tsx`.
...
Google sign-in needs three public OAuth identifiers in `.env.local`: `EXPO_PUBLIC_GOOGLE_WEB_CLIENT_ID` (the one Firebase mints id tokens for), `EXPO_PUBLIC_GOOGLE_IOS_CLIENT_ID`, and `EXPO_PUBLIC_GOOGLE_IOS_URL_SCHEME` (the reversed iOS client id, read by `app.config.ts` at **build** time — changing it requires a rebuild). Leave them empty and the Google button hides itself; the rest of the app still runs.
