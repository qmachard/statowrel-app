import { REFERRAL_WELCOME_STATFLOUZZ_BONUS } from '@statowrel/models';
 ...
 *
 * The **newcomer's** bonus is what the message quotes, never the sender's
 * larger one: the reader is being asked for a favour, and a favour that opens
...
export const shareInvite = async (username: string): Promise<void> => {
  const result = await Share.share({ message: shareMessage(username, REFERRAL_WELCOME_STATFLOUZZ_BONUS) });
