...
- `src/collections/v1_daily_question_answers.ts` — the read-only `v1_daily_question_answers` sub-collection, wired into `v1_questions` via `subcollections`. A sub-collection gets its own file like any other.
