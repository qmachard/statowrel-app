# StatOwrel 1.2.0 — notes de version

| Champ | Valeur |
|---|---|
| Version | **1.2.0** (`apps/app/package.json` et `apps/app/app.config.ts` — **déjà bumpée par la rc1**, rien à changer) |
| Build iOS | **11** |
| Build Android | **11** |
| Date | 2026-09-08 |
| Nature | **release finale** |
| Commit taggé | **`482926e`** — le commit de `1.2.0-rc2`, et **pas la tête de `main`** |
| Version précédente | **1.1.0** (builds iOS 10 / Android 9), taggée le 2026-09-03 sur `b2d4db5` — `release-notes-1.1.0.md` |
| Pre-releases | `1.2.0-rc1` (`82f9234`) et `1.2.0-rc2` (`482926e`), taggées le 2026-09-04 |
...
Cette note est la **note de soumission de référence de la 1.2.0** : elle consolide
`release-notes-1.2.0-rc1.md` et `release-notes-1.2.0-rc2.md`, et couvre les 34 commits qui séparent
`b2d4db5` (1.1.0) de `482926e` (1.2.0-rc2). Les deux notes de rc restent lisibles pour le détail
commit par commit ; tout ce qui doit être vrai avant de soumettre est ici.

> ⚠️ **Le périmètre de cette version s'arrête à `482926e`.** `main` a avancé depuis : l'écran d'un
> pote et son score de compatibilité, les onglets du Menu, le récap Instagram et le parrainage y
> sont déjà, et **rien de tout cela n'est dans la 1.2.0**. C'est pourquoi le tag est posé sur le
> commit de la rc2 et non sur la tête de `main` — le binaire soumis doit être celui qui a été
> recetté. Ces cinq chantiers partent dans la 1.3.0.

Le passage en **mineure** est justifié : la 1.2.0 ouvre une fonctionnalité entière — le **joker**
(`docs/prd.md` §4.8), qui change la règle la plus structurante du produit, celle de la série — et
intègre pour la première fois un **SDK de mesure d'usage**, ce qui déplace une déclaration de
confidentialité.
...
## Ce que la 1.2.0 apporte depuis la 1.1.0

### Dans le binaire

| # | Changement | Effet sur la soumission |
|---|---|---|
| 1 | **Le joker** (`4db85fb` → `0f288d6`) — passer la question du jour sans y répondre, pour 20§, en gardant sa série. Illimité tant que le solde suit, réservé à la journée en cours | **La nouveauté de cette version.** Elle contredit une phrase publiée de la description (« pas de joker ») : `docs/store-listing.md` est corrigé, la fiche en ligne est à corriger le jour de la mise à jour |
...
| 7 | **`AD_ID` retirée du manifeste Android** (`b28902c`) — `android.blockedPermissions`, qu'Expo traduit en `tools:node="remove"` sur l'élément `uses-permission` | **Débloque l'upload sur la Play Console.** Aucune perte fonctionnelle : `google_analytics_adid_collection_enabled: false` empêchait déjà la lecture de l'AdID ; c'est la permission qui mentait, pas la déclaration |
| 8 | **Le joker d'un pote est visible** dans la liste d'amis de la feuille du jour (`cbf905e`) | Se raconte avec le joker |
| 9 | **`ChartBar` au lieu de `BarChart3`** (`b84408d`) — nommage d'icône `lucide` v1.33 | Hors fiche |

### Hors binaire — backend, règles, site et outillage

| # | Changement | Conséquence |
|---|---|---|
| 10 | **`questions-useJoker`** — le callable qui débite 20§ et écrit le joker sur `v1_daily_question_answers/{uid}` en une transaction, seule porte possible | `deploy:functions` **et** `deploy:firestore` obligatoires |
| 11 | **`users-onUserCreated` → `grantInitialBalance`** (`380af59`) — verse les 50§ côté serveur au profil qu'un client ≤ 1.1.0 a ouvert à 0, idempotent sans marqueur (les trois compteurs du portefeuille *sont* le marqueur) | `deploy:functions` |
| 12 | **`firestore.rules`** — `hasAnswerShape()` refuse toute réponse cliente portant `is_joker: true` ; `startsWithInitialBalance()` remplace `startsEmpty()` et **tolère les deux ouvertures, 0 et 50** | `deploy:firestore`. Sans le déploiement, **le joker est refusé et l'onboarding d'un client 1.2.0 aussi** |
| 13 | **Le joker rejoint `v1_daily_question_answers`** (`b106da1`) — une seule collection au lieu de deux, donc une seule lecture par ami sur la feuille du jour et sur la relance de 18h | `deploy:functions` — `onAnswerCreated` branche sur `is_joker` |
| 14 | **Page de présentation devenue page marketing** (`481fc14`, `2cc4243`) — `apps/admin/index.html` reconstruite autour du carrousel d'onboarding : son texte, ses visuels portés en HTML/CSS, une bande « comment ça marche » en trois temps, les boutons App Store / Google Play, les métadonnées OpenGraph / Twitter et un repli `prefers-reduced-motion`. La section « On te prévient » est retirée, ce qui ramène la page aux quatre sections que son chapô annonce | `npm run deploy:admin:production` — **ce qui contredit la rc1**, dont la checklist disait « rien à déployer côté console » |
| 15 | **`npm run backfill-statcoins` devient `backfill-statflouzz`** (`93cfa57`) | Le nom change, le comportement non |
| 16 | **`npm run backfill-initial-balance`** (`380af59`) — verse les 50§ aux profils existants qui n'ont jamais touché leur portefeuille | À passer une fois, après le déploiement des functions |
| 17 | **`docs/analytics.md`** — plan de taggage, source unique des événements | Hors produit, mais **c'est la pièce que la déclaration de confidentialité cite** |
| 18 | **Ports des émulateurs déplacés** (`731696e`, `5b147a7`) — auth 9098, functions 5002, firestore 8082, hosting 5001, UI 4001, storage 9198 ; Metro sur 8083 | Développement seulement. Un `.env.local` ou un script qui pointait les anciens ports est à corriger |
...
- **`AD_ID` est un changement natif** : il prend un build, jamais une mise à jour OTA. **Un AAB
  produit avant `b28902c` porte encore la permission et sera refusé.**
- **Les deux boutons de store de la page marketing pointent des fiches qui n'existent pas encore** —
  `apps.apple.com/fr/app/statowrel/id6803561031` et le listing Play répondent 404 tant que l'app
  n'est pas publiée. À trancher : déployer la page après la mise en ligne, ou masquer les boutons.
- **Il n'y a pas d'`og:image`.** `twitter:card` est `summary`, ce qui est cohérent, mais un lien
  partagé n'affiche aucun visuel.
...
Aucune version n'ayant encore été publiée sur les stores, **le texte A est celui à retenir.**
...
>
> ⚠️ **Ne rien annoncer sur la correction `AD_ID`.** Retirer une permission que l'utilisateur n'a
> jamais vue apparaître ne se raconte pas — et l'annoncer attirerait l'attention sur une permission
> publicitaire dans une app qui n'en a pas.
>
> ⚠️ **Ne rien annoncer de ce qui n'est pas dans le binaire taggé.** La compatibilité entre potes,
> la fiche d'un ami, les onglets du Menu et le parrainage sont sur `main` mais **hors de cette
> version**. Les décrire ici serait décrire une fonctionnalité absente : motif de rejet 2.3.1.
>
> ⚠️ **Ne rien ajouter sur le partage du résultat.** Il n'existe toujours pas dans le binaire
> (`docs/production-checklist.md` §1.3).
>
...
La description de la 1.1.0 avec **une correction obligatoire et un bloc en plus**. La correction est
celle de `TA SÉRIE` : elle disait « pas de joker », ce qui devient faux avec cette version.
...
Un point à garder en tête côté Android : la déclaration de la Play Console reste **« l'app n'utilise
pas d'identifiant publicitaire »**. C'est ce que le binaire dit désormais aussi.
...
Analytics ne lit ni l'IDFA ni l'AdID, et la permission `AD_ID` est retirée du manifeste Android).

---

## Plan de test QA interne

À exécuter sur le build iOS **11** et le build Android **11**, tirés du commit `482926e`, profil
`production`, sur **appareil physique** — pas sur simulateur : les notifications push et la
connexion Apple ne fonctionnent pas autrement.
...
- [ ] Le tap ouvre une **alerte native de confirmation** avant toute dépense — « Annuler » ne
      dépense rien
- [ ] Confirmation : 20§ prélevés, la feuille bascule sur « Tu as utilisé un JOKER », la série avance
...
- [ ] Un pote ayant joué son joker apparaît dans la liste d'amis de la feuille du jour, avec le même
      violet
...
### StatFlouzz, en-tête et bouton de proposition

- [ ] Plus aucune occurrence de « StatCoin » dans l'interface — carte streak, en-tête, formulaire de
      proposition, joker, « Mes questions »
...
- [ ] Il **reste visible** pendant le défilement de l'écran Stats, et se met à jour tout seul après
      une réponse, un joker ou une proposition
- [ ] Sous le calendrier, « Poser une question » est **nu**, pleine largeur, prix en `trailingLabel`
- [ ] Solde insuffisant : le bouton est grisé **mais tapable**, et l'alerte nomme le manque

### Firebase Analytics et `AD_ID`

- [ ] Dev client avec `EXPO_PUBLIC_ANALYTICS_FORCE_ENABLED=true` : les **8 événements** custom et
      `screen_view` remontent dans **DebugView** — protocole dans `docs/analytics.md` §8
...
- [ ] **Il n'y a aucune ligne d'opt-out dans le Menu** — c'est volontaire (`20978f5`)
- [ ] Le nouveau dev client est bien reconstruit — `@react-native-firebase/analytics` est un module
      natif
- [ ] `npx expo prebuild --platform android` : le manifeste généré porte
      `com.google.android.gms.permission.AD_ID` avec `tools:node="remove"`
- [ ] Sur l'AAB produit par EAS, la permission **n'apparaît pas** (`bundletool dump manifest`, ou
      l'onglet des permissions de la Play Console après upload)
- [ ] **L'upload sur la piste interne Play passe** — le seul test qui prouve le correctif
- [ ] La déclaration de la Play Console est restée sur « n'utilise pas l'identifiant publicitaire »
- [ ] Analytics remonte toujours dans DebugView **après** le blocage de la permission
...
- [ ] Un joker écrit bien un document dans `v1_daily_question_answers` avec `is_joker: true` et un
      `option_id` vide
- [ ] `v1_user_calendar_months.jokers.{DD}` est renseigné par le trigger, et `days.{DD}` ne l'est pas
      pour le même jour
- [ ] Les compteurs `friend_answer_counts` des potes s'incrémentent aussi sur un joker

**Page marketing** (`481fc14`, `2cc4243`)

- [ ] `npm run deploy:admin:production` passé, puis la racine du site en navigation privée
- [ ] Les **quatre** sections annoncées par le chapô s'affichent, visuels compris — et **plus** de
      section « On te prévient » ni de cloche
- [ ] La bande « comment ça marche » en trois temps est lisible sur mobile étroit
- [ ] Les boutons App Store et Google Play sont présents dans le héros **et** dans le CTA de fin — et
      **on a tranché ce qu'ils font tant que les fiches n'existent pas**
- [ ] `prefers-reduced-motion` : les maquettes inclinées se posent à plat
- [ ] Un lien partagé affiche bien titre et description — et **on a constaté l'absence de visuel**,
      faute d'`og:image`
- [ ] `/admin/` ouvre toujours la console de modération, et le catch-all sert bien la page marketing
- [ ] Les cinq pages légales répondent toujours
...
Google Android) et deux cases nées de la fusion des collections :

- [ ] La feuille du jour ne fait **qu'une lecture par ami** — le joker et la réponse vivant désormais
...
Et une dernière, avant de builder :

- [ ] `npm run typecheck`, `npm run lint` et `npm run build` verts **sur `482926e`** — la CI les
      passe sur chaque PR, mais rien ne les rejoue sur le tag qu'on soumet
...
```
94 caractères. Inchangés depuis la 1.0.0 — le joker n'apporte aucun terme de recherche : personne ne
cherche « joker » pour trouver une app de question du jour.
...
Ce qui doit être vrai **avant** de soumettre les builds tirés de `482926e`.

### Bloquants store encore ouverts

- [ ] 🔴 **Consentement RGPD / CNIL pour Firebase Analytics** — *propre à cette version*. La CNIL
      exige un consentement explicite pour tout traceur qui n'est pas « strictement nécessaire » ;
      Firebase Analytics en est un. Le wrapper expose déjà `setEnabled(bool)` pour être branché sur
      une couche de consentement, mais **cette couche n'existe pas** : elle a été sortie de la
      version exprès (`20978f5`). Deux issues, une seule à choisir : livrer le bandeau + le drapeau
      persisté + le gate, **ou** contraindre la release production à ne rien envoyer.
      `docs/production-checklist.md` §6
...
- [ ] **Signalement dans l'app** — guideline 1.2, `docs/production-checklist.md` §2.3. Toujours
      aucune collection `v1_user_reports`, aucune action « Signaler ». **Le seul bloquant dur qui
      reste côté code produit**
- [ ] **La fiche en ligne dit « pas de joker »** — la phrase de `TA SÉRIE` doit être corrigée dans
      les deux stores **en même temps** que la mise à jour
- [ ] **Les deux boutons de store de la page marketing pointent des fiches inexistantes** — déployer
      la page après la publication, ou masquer les boutons d'ici là
...
- [ ] URLs de confidentialité, de support et des normes de sécurité des enfants renseignées dans les
      deux consoles
- [ ] Compte de démonstration créé sur la production, avec des journées répondues, deux amis acceptés
      **et au moins 120 StatFlouzz**, puis renseigné dans les deux consoles
...
- [x] **Bouton de proposition qui explique ce qui manque** *(nouveau en 1.2.0)*
- [x] **Firebase Analytics + plan de taggage** `docs/analytics.md` *(nouveau en 1.2.0)*
- [x] **`is_joker` refusé aux clients par les règles** *(nouveau en 1.2.0)*
- [x] **`AD_ID` bloquée dans le manifeste Android** *(nouveau en 1.2.0)*
- [x] **Page marketing** à la racine du site, avec métadonnées de partage *(nouveau en 1.2.0)*
...
1. - [ ] `npm run deploy:firestore:production` — **les règles d'abord** :
        `startsWithInitialBalance()` est ce qui laisse un client 1.2.0 ouvrir un profil, et
        `hasAnswerShape()` ce qui ferme `is_joker` aux clients. Aucun index nouveau cette fois
...
4. - [ ] `npm run deploy:admin:production` — la page marketing. À caler selon ce qu'on décide des
        boutons de store
...
7. - [ ] Seulement ensuite : `npm run build:prod:ios` / `build:prod:android` **depuis `482926e`**,
        puis `submit:prod` — et **un AAB antérieur à `b28902c` est à jeter**, il porte encore `AD_ID`

> ⚠️ **Attention au déploiement backend depuis `main`.** `main` porte déjà des règles et des
> callables qui n'appartiennent pas à cette version. Déployer les functions et les règles **depuis le
> tag `1.2.0`** garde le backend aligné sur le binaire soumis ; déployer depuis `main` fait entrer
> en production du code qu'aucun binaire publié n'appelle. Ce n'est pas dangereux en soi — rien de
> ce qui est sur `main` ne casse un client 1.2.0 — mais c'est un choix à faire sciemment.
...
- [ ] Projet Firebase de développement séparé de la production (`.firebaserc` pointe les deux alias
      ...
      plus gênant qu'avant** : développement et usage réel atterrissent dans le même GA4
...
- [ ] **Au moins 90 questions approuvées** dans le pot — un jour sans question casse la série de tout
      le monde
...
- [ ] **Captures à refaire, pas seulement à compléter** : l'en-tête de l'écran Stats a changé et la
      ...
      périmée
- [ ] Une capture pour le joker — la feuille « Tu as utilisé un JOKER » ou la case violette
- [ ] **Image de partage (`og:image`)** pour la page marketing — 1200×630, aux couleurs de la marque
- [ ] Bannière Play 1024×500, icônes 1024×1024 (iOS) et 512×512 (Play)

### Recette

- [ ] Plan de test QA ci-dessus passé en entier, **après** les déploiements
- [ ] Passé sur iOS **et** Android, sur appareil physique
