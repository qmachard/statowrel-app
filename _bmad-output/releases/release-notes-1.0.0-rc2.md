# StatOwrel 1.0.0-rc2 — notes de version

| Champ | Valeur |
|---|---|
| Version | **1.0.0-rc2** (`version` applicative : `1.0.0`, inchangée depuis la rc1) |
| Build iOS | non figé — la rc précède le build TestFlight (`autoIncrement` + `appVersionSource: "remote"` dans `apps/app/eas.json`) |
| Date | 2026-08-21 |
| Nature | **pre-release** — seconde release candidate, aucune version publiée avant elle |
...
Cette note **remplace** `release-notes-1.0.0-rc1.md` pour la soumission. Les textes App Store sont
inchangés — aucune fonctionnalité produit ne s'est ajoutée depuis la rc1, et la fiche décrit déjà
exactement ce que le binaire fait. Ce qui bouge est **hors fiche** : deux bloquants store fermés,
un `PERMISSION_DENIED` corrigé, et un interrupteur de notifications ajouté dans le Menu.
...
|---|---|---|
| 1 | Politique de confidentialité **publiée** sur `/legal/confidentialite` | Ferme le bloquant §2.2 — l'URL exigée par les deux consoles existe |
| 2 | Page d'assistance **publiée** sur `/legal/assistance` | Ferme l'URL de support exigée par les deux stores, et le « contact avec l'éditeur » de la guideline 1.2 |
| 3 | Identité de l'éditeur complétée sur les quatre pages | Plus aucun `À COMPLÉTER` en ligne — un examinateur ne tombe plus sur un gabarit |
| 4 | Règle Firestore : `counted_at` lu via `get('counted_at', null)` | **Correctif dur** — les builds antérieurs au champ ne pouvaient plus répondre du tout |
| 5 | Permission de notification redemandable depuis le Menu | Le parcours de rc1 ne l'offrait qu'au carrousel d'accueil, donc jamais à un compte existant |

Les URLs sont servies par le même Hosting que la console de modération
(`https://statowrel-app.web.app`, à remplacer si un domaine propre est choisi — `docs/store-listing.md` §4) :

| Page | URL |
|---|---|
| Politique de confidentialité | `https://statowrel-app.web.app/legal/confidentialite` |
| Assistance / support | `https://statowrel-app.web.app/legal/assistance` |
| CGU | `https://statowrel-app.web.app/legal/cgu` |
| Mentions légales | `https://statowrel-app.web.app/legal/mentions-legales` |

> ⚠️ Ces quatre pages ne sont en ligne qu'à hauteur du dernier `npm run deploy:admin:production`.
> Les réécritures Hosting ajoutées à cette rc (`firebase.json`) ne prennent effet qu'à ce
> déploiement : sans lui, une URL propre répond **200 avec la console de modération**, ce qui ne
> ressemble pas à une panne de routage et passerait inaperçu.
...
Champ modifiable **sans nouvelle soumission**. Inchangé depuis la rc1, la première option reste retenue.
...
**Inchangé depuis la rc1** — c'est toujours la première version publiée, et rien de ce qui s'est
ajouté depuis n'est une fonctionnalité qu'on annonce à un utilisateur qui découvre l'app.
...
Inchangée depuis la rc1 : `docs/store-listing.md` §1.4 augmenté du bloc `LA NOTIFICATION DE 7H`,
désormais honoré par le code.
...
**Ce qui a changé par rapport aux notes de la rc1, et pourquoi :**

- Un paragraphe **« SIGNALER UN UTILISATEUR »** est **rétabli**, mais sur le seul chemin qui existe
  réellement : l'adresse de l'éditeur, via la page d'assistance. La rc1 l'avait retiré faute de
  chemin vérifiable. **Le signalement in-app n'existe toujours pas** (`docs/production-checklist.md`
  §2.3) — ne jamais écrire ici qu'il est dans l'app.
- Le paragraphe **notifications** mentionne le nouvel interrupteur du Menu : un examinateur qui
  refuse la permission au carrousel doit pouvoir la retrouver.
- Le paragraphe **blocage** est explicité dans la section contenu généré : retirer un ami *est* le
  blocage attendu par la guideline 1.2.
- Les **URLs légales** sont données en clair, politique de confidentialité comprise — elles n'étaient
  pas publiées à la rc1.
...
**Réponse depuis un build antérieur (le correctif `counted_at`)**

- [ ] Sur un téléphone portant le **build installé avant la rc1**, répondre à la question du jour :
      la réponse passe, **plus aucun `PERMISSION_DENIED`**
- [ ] Sur le build de la rc2, répondre : la réponse passe également
- [ ] Un client qui pré-remplirait `counted_at` est toujours refusé (vérifiable en émulateur)
- [ ] Les règles corrigées sont **déployées** (`npm run deploy:firestore:production`) — le correctif
      est côté serveur, aucun build ne le porte

**Permission de notification redemandée**

- [ ] Compte créé **avant** le carrousel (ou carrousel passé) : au lancement, une alerte native dit
      ce qui sera envoyé, puis lève la boîte système
- [ ] Refuser : l'alerte ne revient **pas** au lancement suivant (drapeau posé sur l'appareil)
- [ ] Compte ayant déjà accepté : aucune alerte, à aucun lancement
- [ ] Menu → bouton notifications : lève la boîte système quand elle est encore disponible
- [ ] Permission refusée définitivement (`canAskAgain` faux) : le bouton ouvre les **réglages système**
- [ ] Accorder la permission depuis les réglages puis revenir dans l'app **sans la relancer** :
      l'état se met à jour, et le jeton Expo est enregistré immédiatement dans `v1_user_devices`
- [ ] Après enregistrement à chaud : `npm run send-test-notification -- --email <compte>` arrive bien

**Pages légales en ligne**

- [ ] `npm run deploy:admin:production` passé, puis les quatre URLs testées **en navigation privée** :
      `/legal/cgu`, `/legal/confidentialite`, `/legal/mentions-legales`, `/legal/assistance`
- [ ] Chacune répond en **HTTPS, sans redirection, sans mur de connexion**, et affiche la page —
      **pas** la console de modération (le symptôme d'une réécriture manquante est un 200 trompeur)
- [ ] Aucun `À COMPLÉTER` visible sur aucune des quatre pages
- [ ] Les trois liens du bas du Menu, de la connexion et de l'inscription ouvrent les bonnes pages,
      **Confidentialité comprise** (nouveau lien de cette rc)
- [ ] `docs/privacy-policy.md` et `/legal/confidentialite` disent la même chose
...
- [ ] Répondre à la démo **puis** se connecter : la réponse est comptée dans les statistiques de la
      question de démonstration, et n'ajoute ni jour au calendrier ni jour à la série
...
- [ ] **Signalement d'un utilisateur dans l'app** — guideline 1.2, `docs/production-checklist.md`
      ...
      éditeur de `/legal/assistance` couvre désormais l'exigence « traitement sous 24 h », mais
      **pas** le mécanisme de signalement lui-même. **Le seul bloquant dur qui reste côté code.**
- [ ] `npm run deploy:admin:production` — les quatre pages légales ne sont en ligne qu'à hauteur du
      dernier déploiement Hosting, réécritures comprises
- [ ] Page web de demande de suppression de compte (exigée par Play, hors de l'app)
- [ ] URLs de confidentialité et de support renseignées dans App Store Connect **et** dans la Play Console
...
- [ ] Relecture juridique des quatre pages légales

### Acquis dans le code à cette rc

- [x] **Politique de confidentialité écrite et servie** sur `/legal/confidentialite` *(nouveau en rc2)*
- [x] **Page d'assistance servie** sur `/legal/assistance`, avec adresse de contact, délais, FAQ,
      suppression de compte, signalement et droits RGPD *(nouveau en rc2)*
- [x] **Identité de l'éditeur complète** sur les quatre pages — Quentin Machard SAS, RCS Laval
      891 303 893 *(nouveau en rc2)*
- [x] **Lien Confidentialité depuis l'app** (`LegalLinks` : Menu, connexion, inscription) *(nouveau en rc2)*
- [x] **Permission de notification redemandable** — alerte une fois par installation pour les comptes
      existants, et interrupteur permanent dans le Menu *(nouveau en rc2)*
- [x] **Règle de création d'une réponse tolérante à `counted_at` absent** — les builds antérieurs
      peuvent de nouveau répondre *(nouveau en rc2, à déployer)*
...
- [ ] `eas env:list --environment production` : les six `EXPO_PUBLIC_FIREBASE_*` présents, aucun
      `*_EMULATOR_*`
- [ ] **`npm run deploy:firestore:production` passé** — porte le correctif `counted_at`, index du
...
- [ ] Delta de la rc2 ci-dessus passé en entier, **correctif `counted_at` compris**
